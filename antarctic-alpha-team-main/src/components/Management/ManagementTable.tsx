...
import { Edit, Trash2, Clock, Calendar as CalendarIcon, ChevronDown, ChevronUp, Info, ChevronLeft, ChevronRight, RotateCcw, ClipboardList } from 'lucide-react'
...
export const ManagementTable = ({ selectedUserId, slotFilter, onEditSlot, onEditStatus, refreshKey, initialWeekStart, onDateChange, showWeekNavigation = true }: ManagementTableProps) => {
  ...
  return (
    <div className={`rounded-2xl ${theme === 'dark' ? 'bg-[#0b1015]' : 'bg-white'} overflow-visible`}>
      {/* Week navigation - hidden, dates are shown in the header above */}
      {false && showWeekNavigation && (
      <div
      ...
      >
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 order-2 sm:order-1">
          ...
          >
            <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: '#4C7F6E' }} />
          </button>
          ...
          >
            ...
            <span className={`hidden sm:inline ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Актуальная</span>
          </button>
          ...
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-xl border transition-all ${theme === 'dark' ? 'bg-white/5 border-white/10 hover:bg-white/10 text-white' : 'bg-white border-gray-200 hover:bg-gray-50 text-gray-900 shadow-sm'}`}
          >
            <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: '#4C7F6E' }} />
          </button>
        </div>

        <div className="flex items-center gap-2 justify-center order-1 sm:order-2">
          <CalendarIcon className="w-5 h-5" style={{ color: '#4C7F6E' }} />
        ...
        <div className="w-full sm:w-auto flex justify-end sm:justify-start order-3">
          ...
          <table className="w-full border-collapse">
            ...
            <thead className="sticky top-0 z-40">
              <tr className={theme === 'dark' ? 'bg-[#0b1015]' : 'bg-white'}>
                <th 
                  colSpan={9} 
                  className={`p-3 text-center border-b ${theme === 'dark' ? 'border-white/5' : 'border-gray-100'}`}
                >
                  <div className="flex items-center justify-center gap-3">
                    <CalendarIcon className="w-5 h-5" style={{ color: '#4C7F6E' }} />
                    <span className={`${headingColor} font-black text-base tracking-tight`}>
                  ...
                  return (
                    <th key={day.toISOString()} className="p-1 min-w-[100px] lg:min-w-[120px]">
                      <div className={`mx-auto w-full p-1.5 rounded-xl border transition-colors ${isToday
                        ? 'bg-[#4C7F6E]/10 border-[#4C7F6E]/50 text-[#4C7F6E]'
            ...
            <tbody className={`divide-y ${theme === 'dark' ? 'divide-white/5' : 'divide-gray-100'}`}>
              {displayUsers.map((user, index) => {
                ...
                return (
                  ...
                  >
                    <td className={`p-2 sticky left-0 z-20 ${rowBg} transition-colors align-middle`}>
                                    ...
                                    <div className={`w-full max-w-[120px] rounded-xl p-2 text-[10px] font-bold transition-all relative shadow-md ${theme === 'dark' ? 'bg-[#1a1f26] border border-white/5 text-gray-200' : 'bg-gray-100 text-gray-700'
                                      ...
                                      <div className="flex flex-col items-center gap-1.5 w-full">
                                        ...
                                        <div className="flex items-center gap-1 flex-wrap justify-center">
                                          ...
                                          {slot.taskId && (
                                            <span className="text-[9px] px-1.5 py-0.5 rounded-full border font-medium truncate bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 border-violet-300 dark:border-violet-700 flex items-center gap-0.5" title={`ID задачи: ${slot.taskId}`}>
