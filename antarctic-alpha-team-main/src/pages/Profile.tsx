            ...
            <div className={`w-full max-w-3xl rounded-3xl p-6 border my-8 ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-gray-200'} shadow-2xl`}>
              ...
              <div className={`space-y-6 max-h-[70vh] overflow-y-auto pr-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                ...
                <p className="text-sm leading-relaxed">
                  Взаимодействие в рамках сообщества подразумевает работу с высокорисковыми активами, что обязывает неукоснительно соблюдать протоколы безопасности транзакционного контура и законодательство.
                </p>

                <p className="text-sm leading-relaxed">
                  Выплаты (компенсации, вознаграждения) аккумулируются и осуществляются исключительно с консолидированного корпоративного счета (мультиподписного кошелька). Любой исходящий транзакционный поток в обязательном порядке подвергается скринингу автоматизированными AML-системами (Anti-Money Laundering). При отсутствии предварительной идентификации получателя (KYC) сервис-монитор классифицирует транзакцию как «анонимный перевод с высоким риском обналичивания/фишинга». Это делегирует платформе законное право на принудительную блокировку (freeze) средств до полного выяснения источника их происхождения. В ряде случаев отсутствие идентификационных данных становится основанием для перманентной конфискации активов без права последующего обжалования.
                </p>

                <p className="text-sm leading-relaxed">
                  Предоставление ИНН и паспортных данных не является формальностью, а квалифицируется как инструмент юридической защиты участника и коллективной безопасности пула. Эти данные позволяют сообществу:
                </p>
                <ul className="list-disc list-inside text-sm space-y-2 ml-2">
                  <li><span className="font-bold">Верифицировать статус:</span> подтвердить, что бенефициар не включен в международные санкционные списки (OFAC, EU), а также в перечни лиц, причастных к экстремизму, терроризму или отмыванию доходов.</li>
                  <li><span className="font-bold">Исключить «дропперство»:</span> предоставить проверяющим органам (включая Росфинмониторинг) доказательства того, что вы являетесь конечным бенефициаром средств, а не подставным транзитным звеном, используемым в схемах по обналичиванию капитала.</li>
                  <li><span className="font-bold">Процессуальное представительство:</span> в случае блокировки выплаты на стороне банка-корреспондента или криптовалютной биржи, сообщество выступает вашим официальным процессуальным представителем. Без ваших идентификационных данных у сообщества отсутствуют законные основания (правосубъектность) для подачи апелляции и защиты ваших имущественных интересов в судебном или досудебном порядке.</li>
                </ul>

                <p className="text-sm leading-relaxed">
                  Участники часто полагают, что прохождения идентификации на стороне их личной биржи или платежной системы достаточно для проведения транзакции. Данное убеждение является юридически ошибочным:
                </p>
                <ul className="list-disc list-inside text-sm space-y-2 ml-2">
                  <li><span className="font-bold">Проблема юридического разрыва:</span> биржа идентифицирует вас как владельца счета, однако не фиксирует легитимную экономическую связь между вашим лицевым счетом и корпоративным кошельком сообщества. Для внутренних систем банка данный трансфер визуализируется как перевод между сторонними, юридически не аффилированными лицами, что является безусловным триггером для включения в зону повышенного финансового мониторинга (PEP-скоринг).</li>
                  <li><span className="font-bold">Статус «верифицированного сообщества»:</span> наличие внутренней базы данных участников с заполненными KYC-анкетами позволяет нам маркировать транзакции как «внутрикорпоративное распределение активов согласно реестру». Это кратно снижает риск-скоринг операций для всего пула участников и нивелирует вероятность запроса подтверждающих документов (compliance-запрос) со стороны эквайера.</li>
                </ul>

                <p className="text-sm leading-relaxed">
                  ФИО, дата рождения и адрес регистрации (прописки) являются критическими атрибутами, без которых техническая и юридическая проверка невозможна:
                </p>
                <ul className="list-disc list-inside text-sm space-y-2 ml-2">
                  <li><span className="font-bold">Определение юрисдикции:</span> место фактического проживания является определяющим фактором для установления налогового резидентства. Сообщество обязано идентифицировать, являетесь ли вы налоговым резидентом РФ или иного государства, для корректного применения норм международных конвенций об избежании двойного налогообложения и корректного расчета агентского НДФЛ (при необходимости).</li>
                  <li><span className="font-bold">Фильтрация по линии ФССП и ФНС:</span> Наличие непогашенных задолженностей (включая алиментные обязательства, налоговые недоимки или кредитные просрочки) делает перевод в ваш адрес сделкой, квалифицируемой как «вывод активов из-под взыскания». Это создает прямой риск наложения обеспечительных мер (ареста) на весь корпоральный кошелек сообщества. Сообщество обязано исключить данный риск для сохранности капитала остальных участников.</li>
                </ul>

                <p className="text-sm leading-relaxed">
                  Предоставленные данные проходят валидацию через внутренние сервисы проверки контрагентов. В случае обнаружения фактов предоставления недействительных или подложных документов, учетная запись участника подлежит перманентной блокировке с конфискацией транзакционного остатка в качестве компенсации юридических издержек сообщества.
                </p>

                <p className="text-sm leading-relaxed">
                  Сообщество строго придерживается политики конфиденциальности (Data Privacy) в рамках Федерального закона № 152-ФЗ:
                </p>
                <ul className="list-disc list-inside text-sm space-y-2 ml-2">
                  <li>Передача данных в ФНС, ФССП, МВД, ФСБ, Росфинмониторинг и иные органы государственной власти осуществляется исключительно в рамках официального межведомственного запроса или при наступлении обстоятельств, прямо предусмотренных действующим законодательством (включая случаи противодействия терроризму).</li>
                  <li>Срок обработки персональных данных ограничен периодом фактического участия в транзакционном контуре сообщества. По окончании взаимодействия либо по истечении 3 (трех) лет с момента последней транзакции данные подлежат анонимизации и уничтожению, за исключением случаев, предусмотренных законодательством.</li>
                </ul>

                <p className="text-sm leading-relaxed">
                  Участник обладает правом не предоставлять запрашиваемый перечень данных, однако принимает на себя следующие имущественные и процессуальные риски:
                </p>
                <ul className="list-disc list-inside text-sm space-y-2 ml-2">
                  <li><span className="font-bold">Отказ в проведении выплаты:</span> если транзакция несет репутационный или финансовый риск блокировки общего корпоративного счета, сообщество оставляет за собой право в одностороннем порядке заморозить выплату до момента полной верификации или до вынесения решения ответственным за сообщество.</li>
                  <li><span className="font-bold">Самостоятельное регулирование споров:</span> в случае блокировки средств на стороне банка-корреспондента или криптовалютной биржи, участник будет вынужден решать проблему в индивидуальном порядке без юридической и процессуальной поддержки сообщества (включая отказ в предоставлении пакета подтверждающих документов для банка).</li>
                  <li><span className="font-bold">Повышенная комиссия (Risk Premium):</span> транзакции, направляемые на неверифицированные или частично верифицированные аккаунты, могут облагаться дополнительным сбором в размере до 30% от суммы перевода. Данная премия обусловлена необходимостью использования альтернативных, усложненных и технически более дорогих каналов вывода, минимизирующих риски блокировки для основного пула.</li>
                </ul>
